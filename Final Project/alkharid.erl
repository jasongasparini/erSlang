...
start(ServerNode) ->
   ...
   io:fwrite("~sStarting Location alkharid (pid ~w) on node ~w.~n",[?id, self(), node()]),
   ...
   register(alkharid, LocPid),
   io:fwrite(", registered as ~w.~n",[alkharid]),
   % Send ourselves to the gameServer.
   io:fwrite("~sNotifying server on node ~w.~n",[?id, ServerNode]),
   {gameServer, ServerNode} ! {node(), registerNewLocation, alkharid},
   % Initialize server monitoring.
   alkharid ! {monitor, ServerNode},
   ok.
...
locationLoop() ->
   receive
      ...
      {_FromNode, enter, GameClientNode}  ->
         io:fwrite("~sA gameClient on ~w is entering alkharid.~n",[?id, GameClientNode]),
...
%--------
describe() ->
   io_lib:format("4. Al Kharid~nAfter a long trip south, you come across the bustling centerpiece of the Kharidian Desert.", []).